...
#这一部分就很神奇，可能由于分段的时间的问题造成了对应各部分能量的一个过约束问题，必须松弛一些才能够跑通？？？？
for index in i:
    m.addConstr(E_i_fc[index] <= P_fc_max * delta_t_i[index]*4)#the distributed energy constrain
...
m.addConstr(m_ESD == ((1 - SOE_i[N-1]) * E_cap) / n_fc_max / H_heat_value )
...
power_plot_function()
SOC_plot_function()
print('the hydrogen consumption is ',(m_fc.x+((1 - SOE_i[N-1].x) * E_cap) / H_heat_value / n_fc_max),'based on distance')