...
for index in i:
    m.addConstr(E_i_seg[index]*n_m - M_Total * g * delta_h - 0.5 * M_Total * (v_i2[index] - v_i2[index-1]) - E_i_drag[index] >= 0, name='conservation of energy1')
    m.addConstr(E_i_seg[index]/n_m - M_Total * g * delta_h - 0.5 * M_Total * (v_i2[index] - v_i2[index-1]) - E_i_drag[index] >= 0, name='conservation of energy2')
...
for index in i:
    m.addConstr(E_i_fc[index] <= P_fc_max * delta_t * 1000)#the distributed energy constrain
    m.addConstr(E_i_ch[index] <= PESD * delta_t * 1000)
    m.addConstr(E_i_dis[index] <= PESD * delta_t * 1000)
